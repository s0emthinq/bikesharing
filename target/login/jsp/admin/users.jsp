    ...
    <c:forEach var="user" items="${usersList}" varStatus="loop">
        <tr>
            ...
            <td>
                <c:choose>
                    <c:when test="${user.status==1}">
                        <a href="http://localhost:8080/controller?command=unblock_user&id=${user.id}" class="btn btn-success">
                        ${unblock}
                        </a>
                    </c:when>
                    <c:when test="${user.status==0}">
                        <a href="http://localhost:8080/controller?command=block_user&id=${user.id}" class="btn btn-danger">
                        ${block}
                        </a>
                    </c:when>
                </c:choose>
